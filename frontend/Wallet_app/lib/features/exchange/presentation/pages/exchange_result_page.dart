...
class ExchangeResultPage extends ConsumerStatefulWidget {
  const ExchangeResultPage({super.key});

  @override
  ConsumerState<ExchangeResultPage> createState() => _ExchangeResultPageState();
}

class _ExchangeResultPageState extends ConsumerState<ExchangeResultPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final form = ref.read(exchangeFormProvider);
      final executionState = ref.read(exchangeExecutionProvider);
  ...
  void _goToTransactions() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(exchangeFormProvider.notifier).reset();
      ref.read(exchangeQuoteProvider.notifier).clearQuoteState();
      ref.read(exchangeExecutionProvider.notifier).reset();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final executionState = ref.watch(exchangeExecutionProvider);
    final result = executionState.result;
    ...
    return OperationResultPage(
      ...
      onRetry: _goToExchangeAgain,
      successTitle: 'تم تنفيذ المصارفة بنجاح',
      pendingTitle: 'المصارفة قيد المعالجة',
      failedTitle: 'تعذر تنفيذ المصارفة',
      successDefaultMessage: (amount, currency) =>
          'تم تنفيذ مصارفة بمبلغ $amount $currency بنجاح.',
      pendingDefaultMessage: (amount, currency) =>
          'تم استلام طلب المصارفة بمبلغ $amount $currency وهو الآن قيد المعالجة.',
      failedDefaultMessage: (amount, currency) =>
          'لم يتم تنفيذ مصارفة مبلغ $amount $currency.',
      retryLabel: 'العودة للمصارفة',
    );
  }
}
